...
\subsection{Multi-Hop Reasoning Preservation}
\label{sec:multihop}
...
\paragraph{Cross-model and cross-architecture validation.}
To test generality beyond the Qwen2.5 family, we evaluate on Microsoft Phi-3-mini-4k-instruct (3.8B parameters, \textbf{Phi architecture}---distinct from Qwen's transformer variant). On the same workload (200 queries, overlap\,$=$\,0.6), optimized ordering achieves p50 TTFT of 35.5\,ms vs.\ 43.4\,ms for retrieval order---an \textbf{18.3\% improvement}. Combined with results on Qwen2.5-0.5B (19.4\%), Qwen2.5-1.5B (20.1\%), and Qwen2.5-7B (32.7\%), ordering benefits generalize across both model scales and architectures.

\paragraph{Top-$k$ sensitivity.}
We sweep the number of retrieved documents $k \in \{3, 5, 7, 10\}$ at overlap\,$=$\,0.6. Ordering improvement is 30.8\% at $k{=}5$ and grows to 61.2\% at $k{=}10$ because longer document sequences create more prefix-sharing opportunities. At $k{=}7$ the improvement is 7.8\%, likely because the overlap-to-$k$ ratio at this point creates fewer long prefix matches. This sweep confirms that larger $k$ (common in production RAG) amplifies the benefit of ordering.
...
\subsection{Real-Corpus Evaluation}
...
We evaluate the full integrated pipeline (\texttt{VLLMCacheProxy}) against bare vLLM and the simple ordering wrapper (Table~\ref{tab:integrated}).

\begin{table}[t]
\caption{Full pipeline comparison (RTX 4090, 200 queries, overlap\,=\,0.6). Integrated proxy matches wrapper latency while adding feedback-based cache synchronization.}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{System} & \textbf{p50 (ms)} & \textbf{Mean (ms)} & \textbf{Overhead} \\
\midrule
Bare vLLM (APC, no ordering) & 59.3 & 55.9 & -- \\
Simple wrapper (trie only)   & 41.1 & 52.6 & 0.066\,ms \\
Integrated proxy (full)      & 41.7 & 54.2 & 0.049\,ms \\
\bottomrule
\end{tabular}%
}
\vspace{-0.5em}
