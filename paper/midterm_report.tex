...
On 100 extractive QA questions (Qwen2.5-7B, answer explicitly embedded in one of 5 passages, strict ``output ONLY the answer'' prompt, \texttt{max\_tokens}$=$16), both orderings achieve \textbf{EM\,$=$\,1.000} with $\Delta$EM\,$=$\,0. This confirms that when the model is capable of extractive QA, reordering does not change the extracted answer.
...
Table~\ref{tab:baselines} compares our greedy ordering against four baselines including recency-based ordering (most recently accessed documents first) and an exhaustive oracle ($k!{=}120$ permutations). Recency ordering provides only 2.3\% improvement over retrieval order because recency does not account for \emph{positional} prefix structure. Our greedy trie walk achieves \textbf{97.5\% of oracle} performance ($+29.1\%$ vs.\ oracle's $+29.7\%$), confirming that the $O(k)$ trie walk is near-optimal for this problem.

\begin{table}[t]
\caption{Ordering baselines (RTX 4090, 195 queries). Greedy trie achieves 97.5\% of oracle performance.}
\label{tab:baselines}
\centering
\small
\resizebox{\columnwidth}{!}{%
...
We emphasize that the contribution is \emph{cache-aware document ordering as a scheduling abstraction}. The trie-based greedy algorithm achieves 97.5\% of oracle performance with $O(k)$ cost, and its online adaptivity enables it to track shifting workloads---making it the practical default for deployment.
