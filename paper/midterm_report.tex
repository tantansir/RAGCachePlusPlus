...
\end{itemize}
While prior work (RAGCache~\citep{jin2025ragcache}, ContextPilot~\citep{contextpilot2025}) has explored cache-aware document scheduling, our contribution is to demonstrate that a \emph{minimal, engine-agnostic} implementation---with near-optimal greedy guarantees (99.7\% of oracle) and online adaptivity---achieves competitive latency reductions (20--33\%) without the complexity of engine-level modifications. We additionally describe a retrieval-inference pipelining optimization as a secondary, orthogonal contribution.
...
We evaluate whether document reordering degrades answer quality using Qwen2.5-7B-Instruct on three progressively rigorous tasks:
\begin{enumerate}
\item \textbf{NQ-Open} (200 questions, generated passages): F1\,$=$\,0.008, $\Delta$F1\,$=$\,0.
\item \textbf{Embedded-answer reading comprehension} (100 questions, answer explicitly in one passage, standard prompt): F1\,$=$\,0.064, $\Delta$F1\,$=$\,0.
\item \textbf{Strict extractive QA} (same 100 questions, ``output ONLY the answer'' prompt, \texttt{max\_tokens}$=$16): \textbf{EM\,$=$\,1.000, $\Delta$EM\,$=$\,0}.
\end{enumerate}
The third configuration achieves \emph{perfect} exact match on both orderings, confirming that when the model is capable of extractive QA, reordering does not change the extracted answer. The quality delta is exactly zero across all three configurations.

\textbf{Interpretation.} The strict extractive QA result (EM\,$=$\,1.000) provides strong evidence for simple factoid retrieval. For more complex reasoning tasks, our 40-example multi-hop evaluation shows $\Delta$EM\,$=$\,0, but a larger-scale study with a standard multi-hop benchmark and paired significance tests would strengthen this claim. We scope our quality guarantee to settings where retrieved passages act as interchangeable evidence snippets; tasks with position-sensitive reasoning may behave differently.
...
\textbf{When does the trie outperform frequency?} On a stationary bursty trace (Table~\ref{tab:baselines}), frequency and trie produce nearly identical p50 TTFT (per-query analysis: same-region $\Delta{=}$1.7\%, region-switch both cold). On the two-phase trace (Table~\ref{tab:coldstart}), the trie provides 2.1$\times$ larger gains during Phase~1 because the trace structure differs: the two-phase trace has higher within-burst overlap, amplifying the trie's positional advantage. The takeaway is that both strategies dramatically outperform cache-unaware ordering; the trie's additional advantage appears when (a)~the workload has strong positional locality within bursts, and (b)~the workload is non-stationary (the trie adapts to distribution shifts while frequency relies on stale global statistics). For purely stationary workloads with moderate overlap, a simple frequency sort may suffice.
...
APC+Frequency & 39.4 & 51.1 & $+30.4$\% \\
APC+Optimized (Ours) & 40.1 & 50.9 & $+29.1$\% \\
APC+Oracle & \textbf{40.0} & \textbf{50.0} & $+29.7$\% \\
\bottomrule
\end{tabular}%
}
\vspace{-0.5em}
...
\paragraph{Online adaptation under distribution shift.}
A key advantage of the trie over stateless heuristics is its ability to reflect \emph{current} cache state. We test this with a two-phase trace: Phase~1 (queries 0--99) targets regions 0--9 with high overlap; Phase~2 (queries 100--199) abruptly shifts to regions 40--49 (zero overlap with Phase~1). Table~\ref{tab:coldstart} shows that during Phase~1, the trie provides \textbf{2.1$\times$ larger improvement} than frequency ordering (30.7\% vs.\ 14.8\% over retrieval) because it captures positional prefix structure, not just document popularity. After the topic shift (Phase~2), both strategies degrade as expected, but the trie maintains a slight edge (15.6\% vs.\ 14.4\%) because it correctly detects no cached prefix for the new topics and falls back to retrieval order, while frequency ordering still prioritizes globally-frequent but now-cold documents.

\begin{table}[t]
\caption{Online adaptation: two-phase workload with topic shift at query 100 (RTX 4090, Qwen2.5-7B).}
\label{tab:coldstart}
\centering
\small
\resizebox{\columnwidth}{!}{%
...
\toprule
 & \multicolumn{3}{c}{\textbf{Phase 1 (stationary)}} & \multicolumn{3}{c}{\textbf{Phase 2 (after shift)}} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
\textbf{Strategy} & \textbf{p50} & \textbf{Mean} & \textbf{vs.\ R.} & \textbf{p50} & \textbf{Mean} & \textbf{vs.\ R.} \\
\midrule
No Cache & 109.7 & 109.9 & -- & 115.0 & 115.0 & -- \\
APC+Retrieval & 109.7 & 105.9 & base & 111.4 & 107.1 & base \\
APC+Frequency & 93.4 & 90.1 & 14.8\% & 95.4 & 96.2 & 14.4\% \\
APC+Optimized & \textbf{76.0} & \textbf{78.2} & \textbf{30.7\%} & \textbf{94.0} & \textbf{93.3} & \textbf{15.6\%} \\
\bottomrule
\end{tabular}%
}
\vspace{-0.5em}
...
Beyond the Qwen2.5 family, we evaluate on Microsoft Phi-3-mini-4k-instruct (3.8B parameters, \textbf{Phi architecture}): ordering achieves \textbf{18.3\% improvement}. Combined with Qwen2.5-0.5B (19.4\%), 1.5B (20.1\%), and 7B (32.7\%), benefits generalize across model scales and architectures. Sweeping $k \in \{3,5,7,10\}$ at overlap\,$=$\,0.6, improvement ranges from 7.8\% to 61.2\%, growing with $k$ because longer document sequences create more prefix-sharing opportunities.
...
To address the concern that all workloads use synthetic documents, we construct a diverse corpus of 1{,}000 Wikipedia-style passages spanning 20 topics (machine learning, astronomy, medicine, history, geography, physics, biology, economics, technology, literature, etc.) and use TF-IDF retrieval to create a query trace with \emph{naturally arising} document overlap---no controlled overlap parameter. Table~\ref{tab:real_corpus} shows that even under very low natural overlap, optimized ordering reduces p50 TTFT by \textbf{10.2\%} (50.5\,ms vs.\ 56.2\,ms). This confirms that the benefit is not an artifact of high controlled overlap: real retrieval patterns produce enough prefix sharing for ordering to help.

\begin{table}[t]
\caption{Real-corpus evaluation with TF-IDF retrieval (RTX 4090, 200 queries, no controlled overlap).}
...
\textbf{Strategy} & \textbf{p50 (ms)} & \textbf{p95 (ms)} & \textbf{Mean (ms)} & \textbf{vs.\ Retr.} \\
\midrule
No Cache      & 57.2 & 67.6 & 60.5 & -- \\
APC+Retrieval & 56.2 & 64.2 & 54.6 & baseline \\
APC+Optimized & \textbf{50.5} & \textbf{60.3} & \textbf{51.3} & +10.2\% \\
\bottomrule
\end{tabular}%
}
\vspace{-0.5em}
...
Under eviction pressure (\texttt{gpu\_mem}\,=\,0.78), the feedback loop achieves 99.5\% prediction accuracy with only 1 mismatch out of 200 queries, triggering 1 trie pruning event. This confirms that the feedback mechanism correctly detects and corrects stale trie entries in practice, though the sequential workload produces few evictions even at reduced memory. We acknowledge that our sequential evaluation does not strongly exercise the feedback mechanism; its primary value is for production multi-tenant deployments where eviction pressure causes frequent trie staleness. Under our sequential workload, 99.5\% accuracy with only 1 pruning event demonstrates that the mechanism works correctly but is rarely needed---a positive sign for the trie's accuracy as a cache proxy in low-eviction regimes.
...
We emphasize that the contribution is \emph{cache-aware document ordering as a scheduling abstraction}, not a specific algorithm. Our experiments show that multiple cache-aware heuristics (frequency, PGDSF-proxy, trie-based) all dramatically outperform cache-unaware ordering. The trie variant is preferred for its online adaptivity (Section~\ref{sec:eval}), near-optimal approximation quality (99.7\% of oracle), and ability to detect distribution shifts---but the scheduling abstraction itself is the primary insight.
