...
\textbf{7B-model validation.} We repeat the quality check with Qwen2.5-7B-Instruct on two tasks: (1)~200 NQ-Open questions with generated passages (F1\,$=$\,0.008, $\Delta$F1\,$=$\,0); and (2)~100 reading-comprehension questions where the answer is explicitly embedded in one of 5 retrieved passages (F1\,$=$\,0.064, $\Delta$F1\,$=$\,0). In both cases, the quality delta between orderings is exactly zero. The 8$\times$ higher F1 on the embedded-answer task confirms the model \emph{can} extract information from context; the unchanged score across orderings strengthens the evidence that reordering does not disrupt retrieval of answer-bearing passages.
...
To test generality beyond the Qwen2.5-7B model, we evaluate on Qwen2.5-0.5B-Instruct (0.49B parameters). On the same workload (100 queries, overlap\,$=$\,0.6), optimized ordering achieves p50 TTFT of 16.4\,ms vs.\ 20.4\,ms for retrieval order---a \textbf{19.4\% improvement}. Combined with the 4060~Ti results on Qwen2.5-1.5B (20.1\%) and the 4090 results on Qwen2.5-7B (32.7\%), this confirms that ordering benefits generalize across model sizes within the Qwen2.5 family.
...
\item \textbf{Model family}: We evaluate Qwen2.5 at three scales (0.5B, 1.5B, 7B) and observe consistent benefits (19--33\%). Validation on architecturally distinct model families (e.g., Llama, Mistral) remains future work.
\end{itemize}
