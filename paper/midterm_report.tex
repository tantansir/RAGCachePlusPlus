...
Concretely, let $T_{\text{ret}}$ be retrieval latency, $T_{\text{cold}}$ be the cold-start TTFT, and $T_{\text{warm}}$ be the TTFT once the system prompt is cached. Without pipelining, end-to-end TTFT is $T_{\text{ret}} + T_{\text{cold}}$. With pipelining, the system-prompt warmup call overlaps with retrieval, yielding $\max(T_{\text{ret}}, T_{\text{pipe}}) + T_{\text{warm}}$, where $T_{\text{pipe}} = T_{\text{cold}} - T_{\text{warm}}$ is the prefill work amortized during retrieval (system-prompt prefill plus associated engine setup). The per-request saving is therefore $\min(T_{\text{ret}}, T_{\text{pipe}})$. After the first request, $T_{\text{pipe}} \rightarrow 0$ once the system prompt is cached, so the benefit is limited to cold starts and cache eviction events. Implementation requires only a \texttt{concurrent.futures.ThreadPoolExecutor} to dispatch the warmup call alongside retrieval---no engine modification is needed.
...
We validate the pipelining design (Section~\ref{sec:design}) by measuring end-to-end wall-clock pipelining (Table~\ref{tab:pipelining_e2e}): we issue a system-prompt warmup call, wait for a simulated retrieval delay, then generate the full prompt with the system prompt cached. Component measurements give $T_{\text{cold}} = 56.8$\,ms and $T_{\text{warm}} = 28.2$\,ms, so the effective pipelined cost is $T_{\text{pipe}} = T_{\text{cold}} - T_{\text{warm}} \approx 28$\,ms (the 21.6\,ms system-prompt prefill plus ${\sim}6$--$7$\,ms of engine setup that the warmup call amortizes). At $T_{\text{ret}} = 50$\,ms, wall-clock TTFT drops from 106.8\,ms (serial = $T_{\text{ret}} + T_{\text{cold}}$) to 78.8\,ms (pipelined = $\max(T_{\text{ret}}, T_{\text{pipe}}) + T_{\text{warm}}$), a 28.0\,ms saving that matches $T_{\text{pipe}}$ almost exactly. Because APC caches the system prompt after the first request, the pipelining benefit is limited to cold starts and cache eviction recovery.

\begin{table}[t]
\caption{End-to-end pipelining: measured wall-clock TTFT (RTX 4090). Serial $=T_{\text{ret}}+T_{\text{cold}}$ ($T_{\text{cold}}=56.8$\,ms); Pipelined $=\max(T_{\text{ret}},T_{\text{pipe}})+T_{\text{warm}}$ with $T_{\text{pipe}}\approx 28$\,ms, $T_{\text{warm}}=28.2$\,ms.}
...
Beyond the Qwen2.5 family, we evaluate on Microsoft Phi-3-mini-4k-instruct (3.8B parameters, \textbf{Phi architecture}): ordering achieves \textbf{18.3\% improvement}. Combined with Qwen2.5-0.5B (19.4\%), 1.5B (20.1\%), and 7B (32.7\%), benefits generalize across model scales and architectures. Sweeping $k \in \{3,5,7,10\}$ at overlap\,$=$\,0.6, improvement scales strongly at larger $k$ ($+60.3$\% at $k{=}10$, $+28.5$\% at $k{=}5$), while small-$k$ gains are noisier ($-0.3$\% at $k{=}3$, $+5.4$\% at $k{=}7$) because the absolute number of cacheable prefix tokens is small; the trend nevertheless confirms that ordering benefits grow with retrieved-document count.
