...
\textbf{Interpretation.} Across all quality evaluations, we observed no measurable quality degradation from document reordering. This is expected when retrieved passages act as interchangeable evidence snippets and the prompt template remains fixed. However, document order can matter for tasks with sequential structure or position-sensitive reasoning, so our results should be interpreted as task-specific rather than a general invariance claim.
...
\textbf{Why frequency ordering matches the trie on stationary workloads.} A per-query analysis confirms that on same-region queries, frequency and trie produce nearly identical p50 TTFT (43.2\,ms vs.\ 42.5\,ms; $\Delta{=}$1.7\%), while on region-switch queries both incur cold-start costs (121.4\,ms vs.\ 121.0\,ms). The Table~\ref{tab:baselines} gap is measurement noise on stationary workloads. However, the two strategies diverge under \emph{non-stationary} conditions: in the two-phase cold-start experiment (Table~\ref{tab:coldstart}), the trie provides 2.1$\times$ larger gains during stationary bursts (30.7\% vs.\ 14.8\%) and adapts immediately after a topic shift. The trie's advantage is \emph{structural}: it captures which document \emph{orderings} are cached (positional prefix information), not just which documents are popular. We view the near-equivalence on stationary workloads as a positive result---it validates that cache-aware ordering is the key mechanism---while the trie's online adaptivity justifies its use in realistic non-stationary settings.
...
Under eviction pressure (\texttt{gpu\_mem}\,=\,0.78), the feedback loop achieves 99.5\% prediction accuracy with only 1 mismatch out of 200 queries, triggering 1 trie pruning event. This confirms that the feedback mechanism correctly detects and corrects stale trie entries in practice, though the sequential workload produces few evictions even at reduced memory. The feedback loop's value increases under sustained multi-tenant load where eviction pressure is higher.
