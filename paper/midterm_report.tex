...
\textbf{7B-model validation.} We repeat the quality check with Qwen2.5-7B-Instruct on three tasks with increasing rigor: (1)~200 NQ-Open questions with generated passages (F1\,$=$\,0.008, $\Delta$F1\,$=$\,0); (2)~100 reading-comprehension questions with embedded answers and standard prompting (F1\,$=$\,0.064, $\Delta$F1\,$=$\,0); and (3)~the same 100 embedded-answer questions with a strict extractive prompt (``output ONLY the answer'') and \texttt{max\_tokens}$=$16 (\textbf{EM\,$=$\,1.000, $\Delta$EM\,$=$\,0}). The third configuration achieves \emph{perfect} exact match on both orderings, confirming that when the model is capable of extractive QA, reordering does not change the extracted answer. The quality delta is exactly zero across all three configurations.
