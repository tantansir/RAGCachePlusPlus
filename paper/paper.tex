...
\begin{itemize}
\item Requires \textbf{no modification} to vLLM or any serving engine---the core ordering policy (trie walk plus prompt build) is ${\sim}$150 LOC; the full integrated system including feedback loop and FastAPI middleware is ${\sim}$1{,}500 LOC (Section~\ref{sec:impl}).
...
\end{enumerate}
The core ordering policy (prompt builder + trie walk) adds approximately 150 LOC on top of a standard RAG pipeline; the full integrated system including the feedback loop, cache manager, and FastAPI middleware is ${\sim}$1{,}500 LOC (detailed in Section~\ref{sec:impl}).
