...
\textbf{Research questions.} We organize the paper around five research questions and state up-front how each is addressed:
\begin{itemize}
\item \textbf{RQ1 --- Is a pure prompt-layer scheduling policy (with zero engine modification) sufficient to capture most of the APC prefix-alignment benefit for bursty RAG workloads?} Addressed by the main TTFT benchmark (Section~\ref{sec:eval}, Table~\ref{tab:main}) and systems-level analysis of overhead, throughput, memory, and concurrent load (Section~\ref{sec:systems}).
\item \textbf{RQ2 --- How close does the $O(k)$ greedy trie walk come to an exhaustive oracle ordering?} Addressed by the prefix-length optimality analysis (Section~\ref{sec:design}, ratio~=~0.997) and the same-run baselines comparison including an exhaustive $k!{=}120$ oracle (Table~\ref{tab:baselines}, TTFT-reduction ratio~=~97.5\%).
\item \textbf{RQ3 --- Does the benefit generalize beyond controlled synthetic overlap---to real-corpus retrieval, multiple model architectures, and varied workload regimes?} Addressed by the Workload Diversity section (NQ-Open, mixed-topic, real-corpus Wikipedia TF-IDF; Section~\ref{sec:nq}) and the cross-architecture evaluation on Qwen2.5 (0.5B/1.5B/7B) and Phi-3-mini (Section~\ref{sec:systems}, Generality).
\item \textbf{RQ4 --- When the knowledge tree is only an approximate cache index, can a lightweight TTFT-based feedback loop keep it synchronized with vLLM's actual cache state without engine instrumentation?} Addressed by the cache-state feedback design (Section~\ref{sec:feedback}), the trie-vs-reality validation under normal and cache-pressure conditions (Section~\ref{sec:trie_valid}, Pearson $r=0.97$), and the integrated-pipeline measurement (Section~\ref{sec:integrated}, 99.5\% prediction accuracy).
\item \textbf{RQ5 --- Does reordering for cache efficiency preserve answer quality, including on multi-hop reasoning where evidence chains matter?} Addressed by the extractive-QA and multi-hop experiments (Section~\ref{sec:eval}: EM $=$ 1.000 extractive; Table~\ref{tab:multihop}: $\Delta$EM = $\Delta$F1 = 0 for both natural and reversed evidence orderings).
\end{itemize}
