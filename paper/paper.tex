...
We release RAGCache++ as a self-contained Python package layered on top of vLLM ($\geq$0.8.0) and FastAPI. The code partitions into three tiers by scope: (i)~the \textbf{core ordering policy} that all main-benchmark gains rely on---trie walk plus prompt assembly---is approximately 150 LOC; (ii)~the \textbf{prompt-layer scheduling logic} in the \texttt{vllm\_integration} module (core policy plus proxy wiring) is below 200 LOC; (iii)~the \textbf{full integrated system} including cache manager, PGDSF policy, feedback loop, spatial index, and FastAPI middleware is approximately 1{,}500 LOC. Source code, benchmarks, and result JSONs are available at \url{https://github.com/tantansir/RAGCachePlusPlus}.
