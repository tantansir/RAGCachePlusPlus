...
\begin{itemize}
\item Requires \textbf{no modification} to vLLM or any serving engine---it operates entirely at the prompt-construction layer (${\sim}$150 lines of Python).
...
We evaluate on two GPU configurations, four model sizes across two architectures, four workloads with varying overlap, and show that simple prompt-layer ordering captures most of the available APC benefit.
...
\textbf{Cross-table baselines.} Because each table in Section~\ref{sec:eval} comes from an independent benchmark run with a freshly loaded LLM instance and its own query trace (necessary to prevent warm-cache confounding), the No-Cache p50 baseline varies slightly across tables on the same 4090 hardware (e.g., 121.3\,ms in Table~\ref{tab:main}, 114.7\,ms in Table~\ref{tab:baselines}, 110.3\,ms in Table~\ref{tab:spatial}). Within each table, percentage improvements are always computed against that table's own APC+Retrieval baseline, so comparisons inside a single table are internally consistent; absolute cross-table comparisons should be read with this run-to-run variance in mind.
...
Table~\ref{tab:baselines} compares our greedy ordering against four baselines including recency-based ordering (most recently accessed documents first) and an exhaustive oracle ($k!{=}120$ permutations). Recency ordering provides only 2.3\% improvement over retrieval order because recency does not account for \emph{positional} prefix structure. Our greedy trie walk achieves \textbf{99\% of oracle} TTFT reduction ($+29.1\%$ vs.\ oracle's $+29.3\%$), confirming that the $O(k)$ trie walk is near-optimal for this problem.

\begin{table}[t]
\caption{Ordering baselines (RTX 4090, 195 queries). Greedy trie achieves 99\% of oracle's TTFT reduction. Recency and Oracle come from a separate run that included those ordering modes; the percentages in the last column are all recomputed against the displayed Retrieval p50=56.6\,ms so rows are internally consistent.}
\label{tab:baselines}
\centering
\small
\resizebox{\columnwidth}{!}{%
...
APC+Recency & 55.3 & 52.8 & $+2.3$\% \\
APC+Optimized (Ours) & \textbf{40.1} & \textbf{50.9} & $+29.1$\% \\
APC+Oracle & 40.0 & 50.0 & $+29.3$\% \\
\bottomrule
\end{tabular}%
}
\vspace{-0.5em}
...
To evaluate RAGCache++ on a workload derived from real questions, we construct a corpus from the NQ-Open dataset\footnote{NQ-Open: \url{https://huggingface.co/datasets/nq_open}} using 500 question-answer pairs. We generate topically-clustered Wikipedia-style passages grouped by answer similarity and retrieve top-5 passages per query using TF-IDF similarity. This produces a high-overlap trace (mean Jaccard~$= 0.88$) reflecting concentrated topical retrieval.

At this high-overlap operating point, APC alone already achieves 63\% TTFT reduction over the same run's no-cache baseline. Ordering adds a modest further 2.3\% improvement. This is consistent with the overlap sensitivity analysis (Figure~\ref{fig:overlap}): at very high overlap, retrieval order already produces near-maximal prefix alignment. The NQ result confirms that our synthetic overlap sweep generalizes to real-question-derived traces---ordering benefits are concentrated in the moderate-overlap regime.
...
\paragraph{Real-corpus evaluation.}
\label{sec:real_corpus}

To address the concern that all workloads use synthetic documents, we construct a diverse corpus of 1{,}000 Wikipedia-style passages spanning 20 topics (machine learning, astronomy, medicine, history, geography, physics, biology, economics, technology, literature, etc.) and use TF-IDF retrieval to create a query trace with \emph{naturally arising} document overlap---no controlled overlap parameter. The resulting mean Jaccard is only $0.06$, characteristic of diverse-topic retrieval. Table~\ref{tab:real_corpus} shows that even under this very low natural overlap, optimized ordering reduces p50 TTFT by \textbf{26.8\%} (24.8\,ms vs.\ 33.9\,ms), and mean TTFT by 20.4\%. This confirms that the benefit is not an artifact of high controlled overlap: real retrieval patterns produce enough prefix sharing for ordering to help.
...
Under eviction pressure (\texttt{gpu\_mem}\,=\,0.78), the feedback loop achieves 99.5\% prediction accuracy with only 1 mismatch out of 200 queries, triggering 1 trie pruning event. This confirms that the feedback mechanism correctly detects and corrects stale trie entries in practice, though the sequential workload produces few evictions even at reduced memory. The feedback loop achieves 99.5\% prediction accuracy, confirming that the trie is an accurate cache proxy and that the feedback mechanism correctly detects the rare stale entries that do occur.
...
We release RAGCache++ as a self-contained Python package layered on top of vLLM ($\geq$0.8.0) and FastAPI. The full system is approximately 1{,}500 lines of Python, of which the prompt-layer scheduling logic that produces all of the latency gains in Section~\ref{sec:eval} is $<$\,200 lines. Source code, benchmarks, and result JSONs are available at \url{https://github.com/tantansir/RAGCachePlusPlus}.
...
\begin{lstlisting}[caption={Greedy ordering (\texttt{prompt\_builder.py}).},label={lst:order}]
...
\item \textbf{Fallback path.} When the trie is empty (cold start) or no document matches a cached child, the algorithm degenerates to retrieval-rank order. This guarantees the proxy is a strict superset of bare APC---never worse than the baseline.
\end{itemize}
...
% Allow looser line breaking (long texttt{...} + \ref{...} runs create
% internal whitespace rivers under strict justification).
\sloppy
\emergencystretch=3em

The project was led by \textbf{Kaizhen Tan}, with \textbf{Rong Gu} and \textbf{Mingyuan Li} each owning specific subsystems and experiment tracks. Module names below refer to Table~\ref{tab:modules}.

\paragraph{Kaizhen Tan (lead).}
\begin{itemize}
\item \textbf{System and code.} Designed the overall architecture and the integration layer: \texttt{vllm\_integration/serving.py} (\texttt{VLLMCacheProxy}, \texttt{CacheStateFeedback}, \texttt{RAGCacheMiddleware}) and \texttt{vllm\_integration/prompt\_builder.py}, plus the core benchmark drivers (\texttt{benchmark\_final.py}, \texttt{benchmark\_systems.py}, \texttt{benchmark\_integrated.py}, \texttt{benchmark\_reviewer.py}). Wired the previously stand-alone cache modules into the live vLLM serving path through the 7-stage \texttt{serve\_request} loop (Listing~\ref{lst:serve}).
\item \textbf{Experiments.} Ran the main TTFT comparison (Table~\ref{tab:main}, Figure~\ref{fig:ttft}), overlap sensitivity sweep (Figure~\ref{fig:overlap}), systems-analysis suite (Tables~\ref{tab:profiling}--\ref{tab:concurrent}, \ref{tab:baselines}), end-to-end pipeline (Tables~\ref{tab:pipelining_e2e},~\ref{tab:e2e}), integrated-pipeline / feedback-loop validation (Section~\ref{sec:integrated}), and the cross-architecture experiment on Phi-3-mini-4k-instruct. Operated the local RTX 4060~Ti machine and managed all AutoDL RTX 4090 sessions.
\item \textbf{Writing and infrastructure.} Primary author of the Abstract, Introduction, Methodology (Sections~\ref{sec:design}--\ref{sec:feedback}), Experiments, and Implementation Details. Produced all figures via \texttt{paper/gen\_figures.py}; set up the GitHub repository (\url{https://github.com/tantansir/RAGCachePlusPlus}); maintained the LaTeX build.
\end{itemize}

\paragraph{Rong Gu.}
\begin{itemize}
\item \textbf{Code.} Owned the cache subsystem: \texttt{cache/knowledge\_tree.py} (trie nodes, \texttt{prefix\_match}, eviction-aware lookup that short-circuits at \texttt{tier="none"} nodes), \texttt{cache/cache\_manager.py} (multi-tier accounting, GPU/host allocators, eviction triggers), \texttt{cache/pgdsf\_policy.py} (PGDSF priority computation), and \texttt{cache/spatial\_index.py} (geohash spatial filter consumed by the geospatial workload).
\item \textbf{Experiments.} Designed and ran the system-validation experiments in Section~\ref{sec:trie_valid}: trie-vs-reality correlation under normal load and under cache pressure, plus the eviction-recovery measurements that characterize the feedback loop's accuracy. Designed the baseline matrix (sorted, recency, oracle, frequency-proxy) used in Table~\ref{tab:baselines} and contributed the greedy-vs-oracle approximation-quality analysis.
\item \textbf{Writing.} Co-authored Background (Section~\ref{sec:background}, especially the APC mechanics and chained-hash analysis), the Knowledge Tree subsection of Methodology, and the Cache-State Feedback Loop subsection (Section~\ref{sec:feedback}).
\end{itemize}

\paragraph{Mingyuan Li.}
\begin{itemize}
\item \textbf{Code.} Owned the workload, retrieval, and high-level controller layers: \texttt{evaluation/workload\_generator.py} (synthetic bursty-overlap, geospatial, NQ-Open, mixed-topic, and Wikipedia-style real-corpus traces), the \texttt{retrieval/} TF-IDF retriever used by the real-corpus and NQ-Open evaluations, and \texttt{serving/rag\_controller.py} (the retrieval-inference pipelining controller behind Section~\ref{sec:design}).
\item \textbf{Experiments.} Designed and ran the answer-quality experiments---extractive QA (EM\,$=$\,1.000, Section~\ref{sec:eval}) and multi-hop reasoning (Table~\ref{tab:multihop})---on Qwen2.5-7B; ran the workload-diversity experiments (NQ-Open, mixed-topic, real-corpus, geospatial; Tables~\ref{tab:spatial},~\ref{tab:real_corpus}, Section~\ref{sec:nq}).
\item \textbf{Writing.} Lead author of Related Work (Section~\ref{sec:background}), Discussion (Section~\ref{sec:discussion}), Conclusion (Section~\ref{sec:conclusion}), and the Limitations and Future Work paragraphs; curated \texttt{paper/references.bib}; wrote the README and helped manage the LaTeX build.
\end{itemize}
...
% Tighten appendix section spacing
